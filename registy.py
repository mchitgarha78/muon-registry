...
from frost_mpc.common.utils import Utils
...
class Registry:
    ...
    async def run(self, args) -> None:
        async with trio.open_nursery() as nursery:
            ...
            if args.operation == 'predefined-party':
                await self.predefined_party_dkg(args)
    ...
    async def random_party_dkg(self, app_name: str, threshold: int,  n: int):
        await self.update_dkg_list()
        deployment_dkg_id = [key for key, value in self.dkg_list.items() if value['app_name'] == 'deployment'][0]
        ...
        commitments_dict = await self.get_commitments(deployment_dkg_key['party'])
